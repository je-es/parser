...
// ╔════════════════════════════════════════ CORE ════════════════════════════════════════╗
    ...
    export class Parser {
        ...
        // ┌──────────────────────────────── INIT ──────────────────────────────┐
            ...
            // Performance
            public memoCache        = new Map<string, unknown>();
        ...
        // ┌──────────────────────────────── MAIN ──────────────────────────────┐
            ...
            private parseWithRecovery(startRule: Rule): void {
                ...
                while (this.index < this.tokens.length && (maxErrors === 0 || this.errors.length < maxErrors)) {
                    ...
                    try {
                        const result : unknown = this.parsePattern(startRule.pattern, startRule);

                        if (result !== null) {
                            console.log(`try to build and save: ${JSON.stringify(result)}`);
                            const processed = startRule.options?.build
                                ? this.safeBuild(startRule.options.build, Array.isArray(result) ? result : [result])
                                : result;
                            const IsArray = Array.isArray(processed);
                            const finalRes : AstNode[] = IsArray ? processed : [processed];
                            this.ast.push(...finalRes);
                        }
            ...
            protected parsePattern(pattern: Pattern, parentRule?: Rule): unknown {
            ...
            private executePattern(pattern: Pattern, parentRule?: Rule, shouldBeSilent?: boolean): unknown {
                // data
                const type = pattern.type;
                const name = pattern.name as string;
                const min  = pattern.min as number || 0;
                const max  = pattern.max as number || Infinity;
                switch (type) {
                    case 'token':
                        return this.parseToken(name, parentRule, shouldBeSilent);
                    case 'rule':
                        return this.parseRule(name, parentRule, shouldBeSilent);
                    case 'repeat':
                        return this.parseRepeat(pattern.pattern as Pattern, min, max, pattern.separator as Pattern | undefined, parentRule, shouldBeSilent);
                    case 'seq':
                        return this.parseSequence(pattern.patterns as Pattern[], parentRule, shouldBeSilent);
                    case 'choice':
                        return this.parseChoice(pattern.patterns as Pattern[], parentRule, shouldBeSilent);
                    case 'optional':
                        return this.parseOptional(pattern.pattern as Pattern, parentRule);
                    default:
                        throw new Error(`Unknown pattern type: ${(pattern as Pattern).type}`);
                }
            }

            private parseToken(tokenName: string, parentRule?: Rule, shouldBeSilent?: boolean): unknown {
                ...
                if (token.kind === tokenName) {
                    ...
                    return {
                        span: consumedToken.span,
                        rule: tokenName,
                        value: {
                            kind: consumedToken.kind,
            ...
            protected parseRule(ruleName: string, parentRule?: Rule, shouldBeSilent?: boolean): unknown {
                ...
                try {
                    ...
                    let finalResult: AstNode[] = Array.isArray(result) ? result : [result];
            ...
            private parseOptional(pattern: Pattern, parentRule?: Rule): AstNode[] | AstNode[][] | AstNode[][] {
                ...
                try {
                    ...
                    const _result : unknown = this.parsePattern(pattern, parentRule);
                    const result : AstNode[] = Array.isArray(_result) ? _result : [_result];
                    ...
                    this.index = startIndex;
                    this.errors = savedErrors;
                    this.log('verbose', `✓ OPTIONAL → [] (exception caught: ${(e as ParseError).msg || e}) @${this.index}`);
                    return [];
                }
            }

            private parseRepeat(pattern: Pattern, min = 0, max = Infinity, separator?: Pattern, parentRule?: Rule, shouldBeSilent?: boolean): AstNode[] {
                this.lastHandledRule = pattern.type;
                ...
                while (results.length < max && this.index < this.tokens.length) {
                    ...
                    try {
                        ...
                        const IsArrayOfArray = Array.isArray(result) && result.length > 0 && Array.isArray(result[0]);
                        const finalRes = IsArrayOfArray ? result[0] as AstNode[] : result as AstNode[];
            ...
            private parseChoice(patterns: Pattern[], parentRule?: Rule, shouldBeSilent?: boolean): unknown {
                ...
                for (let patternIndex = 0; patternIndex < patterns.length; patternIndex++) {
                    ...
                    try {
                        ...
                        if (!bestResult || (progress >= bestResult.progress && (error as ParseError).failedAt > bestResult.failedAt)) {
                            bestResult = {
            ...
            private parseSequence(patterns: Pattern[], parentRule?: Rule, shouldBeSilent?: boolean): unknown {
            ...
            private safeBuild(buildFn: (matches: AstNode[]) => AstNode, matches: AstNode | AstNode[]): AstNode | null {
                try {
                    const IsArray = Array.isArray(matches);
                    console.log(`try to build: ${JSON.stringify(matches)}`);
                    return buildFn(IsArray ? matches : [matches]);
                } catch (error) {
        ...
        // ┌──────────────────────────────── HELP ──────────────────────────────┐
            ...
            private extractRuleReferences(pattern: Pattern): string[] {
                ...
                switch (type) {
                    case 'rule':
                        ...
                        break;
                    case 'repeat':
                        refs.push(...this.extractRuleReferences(pattern.pattern as Pattern));
                        if (pattern.separator) {
                            refs.push(...this.extractRuleReferences(pattern.separator as Pattern));
                        }
                        break;
                    case 'optional':
                        refs.push(...this.extractRuleReferences(pattern.pattern as Pattern));
                    ...
                    case 'choice':
                        if (pattern.patterns) {
                            for (const p of pattern.patterns as Pattern[]) {
            ...
            private deepClone(obj: unknown): unknown {
                ...
                if ('type' in obj || 'span' in obj || 'value' in obj) {
            ...
            private patternToString(pattern: Pattern): string {
                switch (pattern.type) {
                    ...
                    case 'rule': return pattern.name as string;
                    case 'repeat': return `${this.patternToString(pattern.pattern as Pattern)}...`;
                    case 'optional': return `${this.patternToString(pattern.pattern as Pattern)}?`;
                    case 'choice': return `choice(${(pattern.patterns as Pattern[]).map((p: unknown) => this.patternToString(p as Pattern)).join('|')})`;
                    case 'seq': return `seq(${(pattern.patterns as Pattern[]).map((p: unknown) => this.patternToString(p as Pattern)).join(' ')})`;
        ...
        // ┌─────────────────────────────── ERROR ──────────────────────────────┐
            ...
            private normalizeError(error: unknown, defaultSpan: Span): ParseError {
        ...
        // ┌─────────────────────────────── CACHE ──────────────────────────────┐
            ...
            private cleanMemoCache(): void {
                ...
                const validEntries = entries.filter(([_key, value]) => {
                    if (now - ((value as MemoEntry).cachedAt || 0) > 1000) {return false;}
                    if ((value as MemoEntry).errorCount !== this.errors.length) {return false;}
                    return true;
                });
            ...
            private createMemoKey(patternType: string, patternData: unknown, position: number, ruleName?: string): string {
                ...
                const type = patternType as string;
                const name = (patternData as Pattern).name as string;
                const min = (patternData as Pattern).min as number || 0;
                const max = (patternData as Pattern).max as number || Infinity;
                ...
                switch (type) {
                    ...
                    case 'repeat':
                        return `${baseKey}:${min}:${max}:${(patternData as Pattern).separator ? 'sep' : 'nosep'}`;
                    case 'seq':
                    case 'choice':
                        { const patternHash = this.hashPatterns(((patternData as Pattern).patterns as Pattern[])  || []);
                        return `${baseKey}:${((patternData as Pattern).patterns as Pattern[])?.length || 0}:${patternHash}`; }
                    default:
                        return baseKey;
                }
            }
            ...
            private getMemoized(key: string): { hit: boolean; result?: AstNode[]; newIndex?: number } {
                if (!this.settings.maxCacheSize || this.memoCache.size >= this.settings.maxCacheSize) {
                    return { hit: false };
                }

                const cached : MemoEntry = this.memoCache.get(key) as MemoEntry;
                if (cached !== undefined) {
            ...
            private isCachedResultValid(cached: MemoEntry): boolean {
                if (typeof cached.newIndex !== 'number' || cached.newIndex < 0) {return false;}
                if (cached.newIndex > this.tokens.length) {return false;}
                return true;
            }

            private memoize(key: string, result: unknown, startIndex: number, endIndex: number): void {
                ...
                const memoEntry : MemoEntry = {
                    result: this.deepClone(result) as AstNode[],
                    newIndex: endIndex,
                    cachedAt: Date.now(),
            ...
            private shouldUseMemoization(pattern: Pattern, _parentRule?: Rule): boolean {
                ...
                const min = pattern.min as number || 0;
                const max = pattern.max as number || 0;

                return pattern.type === 'rule' ||
                    pattern.type === 'choice' ||
                    pattern.type === 'seq' ||
                    pattern.type === 'optional' ||
                    (pattern.type === 'repeat' && (min > 1 || max > 1));
            }
...
// ╔════════════════════════════════════════ MAIN ════════════════════════════════════════╗
    ...
    export function getMatchesSpan(matches: AstNode[]): Span {
        ...
        for (const match of matches) {
            ...
            if(Array.isArray(match)) {
                for (const subMatch of match) {
                    if (subMatch && typeof subMatch === 'object' && 'span' in subMatch) {
                        if (!firstSpan) {
                            firstSpan = subMatch.span;
                        }
                        lastSpan = subMatch.span;
                    }
                }
                continue;
            }
            ...
            // is element ?
            if (match && typeof match === 'object' && 'span' in match) {
                if (!firstSpan) {
                    firstSpan = match.span as Span;
                }
                lastSpan = match.span as Span;
            }
        }
    ...
    export function isOptionalPassed(res: AstNode[]) : boolean {
        return res.length > 0;
    }

    export function getOptional(res: AstNode[], ret: unknown = undefined, index = 0, isSeq = false) : unknown {
        if(!isOptionalPassed(res)) {return ret;}
        if(isSeq && Array.isArray(res)) {
